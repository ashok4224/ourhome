...
import { 
  X, MapPin, GraduationCap, Heart, Train, Star, Compass, 
  Map as MapIcon, Navigation, Clock, Info, CheckCircle, Eye, ArrowRight
} from 'lucide-react';
...
export const NearbyInfrastructureModal: React.FC<NearbyInfrastructureModalProps> = ({
  property,
  isOpen,
  onClose
}) => {
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'school' | 'hospital' | 'transit'>('all');
  ...
  const getCategoryColor = (type: 'school' | 'hospital' | 'transit') => {
    switch (type) {
      case 'school': return 'emerald';
      case 'hospital': return 'rose';
      case 'transit': return 'sky';
    }
  };

  const getCategoryIcon = (type: 'school' | 'hospital' | 'transit', className = "w-4 h-4") => {
    switch (type) {
      case 'school': return <GraduationCap className={className} />;
      case 'hospital': return <Heart className={className} />;
      ...
      <div className="fixed inset-0 z-55 flex items-center justify-center p-4">
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1 flex items-center gap-1 leading-none">
                  <MapPin className="w-3.5 h-3.5 text-rose-500" /> {property.location}
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 min-h-0 flex flex-col md:flex-row relative">
            ...
            <div className={`w-full md:w-[360px] border-r border-slate-100 flex flex-col bg-slate-50/30 overflow-hidden ${activeTab === 'list' ? 'flex' : 'hidden md:flex'}`}>
              
              {/* Category Segmented Controllers */}
              <div className="p-4 border-b border-slate-100 bg-white/70 space-y-2">
                <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block font-bold">Category Filters</span>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2.5 max-h-full">
                {filteredHubs.length === 0 ? (
                  <div className="text-center py-12 text-slate-400 px-4">
                    ...
                    <p className="text-xs font-mono">No nodes matching this category filtered nearby.</p>
                  </div>
                ) : (
                      ...
                      >
                        ...
                        <div className="flex gap-2.5 items-start">
                          ...
                          <div className={`w-8 h-8 rounded-xl shrink-0 flex items-center justify-center ${
                            hub.type === 'school' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' :
                            hub.type === 'hospital' ? 'bg-rose-50 text-rose-600 border border-rose-105' :
                          ...
                          <div className="space-y-1 pr-12">
                            ...
                            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 font-mono text-[10px] text-slate-500">
                              <span className="font-bold text-slate-900">{hub.distance}</span>
                              <span className="text-slate-300">•</span>
                              <span className="flex items-center gap-0.5"><Clock className="w-3 h-3" /> {hub.time}</span>
                            </div>
              ...
              <div className="p-3.5 bg-slate-100 border-t border-slate-200/50 text-[10px] font-mono text-slate-500 flex items-center gap-2">
                ...
                <span>Locality infrastructure verified under TS-RERA audit standards.</span>
              </div>
            </div>

            {/* Simulated Vector Map Panel (Map view) */}
            <div className={`flex-1 relative bg-slate-50 flex flex-col justify-between overflow-hidden ${activeTab === 'map' ? 'flex' : 'hidden md:flex'}`}>
              ...
              <div className="absolute inset-0 select-none z-0">
                ...
                <svg className="w-full h-full opacity-65" viewBox="0 0 500 400" preserveAspectRatio="none">
                  {/* Water bodies */}
                  <path d="M 0,20 Q 150,110 320,60 T 500,80 L 500,0 L 0,0 Z" fill="#e0f2fe" opacity="0.6"/>
                  
                  {/* Parks / Forest Zones */}
                  <rect x="20" y="120" width="80" height="90" rx="12" fill="#ecfdf5" opacity="0.7" />
                  <rect x="380" y="270" width="100" height="80" rx="16" fill="#ecfdf5" opacity="0.7" />
                  <ellipse cx="440" cy="80" rx="40" ry="25" fill="#ecfdf5" opacity="0.5" />
                  ...
                  <line x1="450" y1="0" x2="450" y2="400" stroke="#f1f5f9" strokeWidth="1" />

                  <line x1="0" y1="80" x2="500" y2="80" stroke="#f1f5f9" strokeWidth="1" />
                  <line x1="0" y1="180" x2="500" y2="180" stroke="#f1f5f9" strokeWidth="1" />
                  <line x1="0" y1="280" x2="500" y2="280" stroke="#f1f5f9" strokeWidth="1" />

                  {/* Major Highway structures (Outer Ring Road, Expressways) */}
                  <path d="M 0,350 C 150,340 280,380 500,320" fill="none" stroke="#cbd5e1" strokeWidth="12" strokeLinecap="round" />
                  <path d="M 0,350 C 150,340 280,380 500,320" fill="none" stroke="#ffffff" strokeWidth="8" strokeLinecap="round" />

                  {/* High Tech Street Grid Layout lines */}
                  <line x1="120" y1="0" x2="160" y2="400" stroke="#e2e8f0" strokeWidth="3" />
                  <line x1="280" y1="0" x2="220" y2="400" stroke="#e2e8f0" strokeWidth="3.5" />
                  
                  <line x1="0" y1="220" x2="500" y2="240" stroke="#e2e8f0" strokeWidth="3" />
                  <line x1="0" y1="140" x2="500" y2="110" stroke="#e2e8f0" strokeWidth="3.5" />

                  <path d="M 120,50 Q 250,200 380,350" fill="none" stroke="#e2e8f0" strokeWidth="4" />
                  <path d="M 50,220 Q 250,200 450,220" fill="none" stroke="#f1f5f9" strokeWidth="6" />
                </svg>
              </div>
              ...
              <div 
                ...
                style={{ left: '50%', top: '50%', width: '310px', height: '310px' }}
              >
                    ...
                    >
                      <div className="bg-slate-900 border border-slate-800 text-white p-2.5 rounded-xl shadow-xl w-48 text-left space-y-1">
                        ...
                        <div className="flex items-center justify-between text-[9px] font-mono text-slate-350 pt-1 border-t border-slate-800/80">
                          <span className="font-bold text-emerald-400">{hub.distance}</span>
                          <span className="flex items-center gap-0.5 text-slate-400"><Clock className="w-2.5 h-2.5" /> {hub.time.split(' ')[0]}m</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {activeHoveredHub ? (
                  <div className="flex items-center gap-3 animate-fade-in w-full">
                    ...
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                      activeHoveredHub.type === 'school' ? 'bg-emerald-50 text-emerald-650' :
                      activeHoveredHub.type === 'hospital' ? 'bg-rose-50 text-rose-650' :
                    ...
                    <div className="text-right shrink-0">
                      <span className="text-[9.5px] text-slate-400 font-mono block">COMMUTE TIME</span>
                      <span className="text-xs font-mono font-bold text-slate-800">{activeHoveredHub.time}</span>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 text-slate-400 font-mono text-xs py-1">
                    ...
                    <span>Hover over any pin indicator to measure real commute vectors.</span>
                  </div>
                )}
              </div>
            </div>
