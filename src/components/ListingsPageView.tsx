...
export function getAppreciationTrend(p: Property): AppreciationData {
  ...
  let demandScore = 75;
  let growthFactorText = "Fueled by steady baseline demand and prime urban access routes.";
  ...
  if (loc.includes('jubilee')) {
    ...
    demandScore = 98;
    growthFactorText = "Backed by generational elite land heritage and premium lifestyle amenities.";
  } else if (loc.includes('financial') || loc.includes('nanakramguda')) {
    rateYoY = 14.2;
    trendStatus = 'hyper';
    trendLabel = 'Hyper-Growth Zone';
    demandScore = 95;
    growthFactorText = "Driven by intense banking hub expansions, tech corridors, and RERA security.";
  } else if (loc.includes('gachibowli')) {
    rateYoY = 12.4;
    trendStatus = 'hyper';
    trendLabel = 'High Demand Hotspot';
    demandScore = 91;
    growthFactorText = "Excellent IT hub proximity, premium co-working demand, and zero vacancy rates.";
  } else if (loc.includes('madhapur')) {
    rateYoY = 9.8;
    trendStatus = 'steady';
    trendLabel = 'Robust Steady Core';
    demandScore = 82;
    growthFactorText = "Established commercial district sustaining steady premium valuation trends.";
  } else if (loc.includes('kondapur')) {
    rateYoY = 7.4;
    trendStatus = 'steady';
    trendLabel = 'Consistent Retail Core';
    demandScore = 70;
    growthFactorText = "Steady residential flow driven by botanical park proximity and retail sectors.";
  } else if (loc.includes('banjara')) {
    rateYoY = 4.2;
    trendStatus = 'stagnant';
    trendLabel = 'Saturated Mature Asset';
    demandScore = 48;
    growthFactorText = "Highly valued area but experiencing stagnant price index movement due to layout density limits.";
  } else {
    ...
    if (idNum % 3 === 0) {
      ...
      demandScore = 35;
      growthFactorText = "Established localized residential sector with stagnant rental index expansion.";
    } else if (idNum % 3 === 1) {
      rateYoY = 11.5;
      trendStatus = 'hyper';
      trendLabel = 'Ascending Hotspot';
      demandScore = 88;
      growthFactorText = "Spurred by fast-track road extensions and fresh corporate tech parks.";
    } else {
      rateYoY = 6.8;
      trendStatus = 'steady';
      trendLabel = 'Moderate Urban Steady';
      demandScore = 65;
      growthFactorText = "Comfortable baseline growth in an established middle-class tech submarket.";
    }
  }
...
export const AppreciationTrendGauge: React.FC<{ property: Property; showDetailedMode?: boolean }> = ({ property, showDetailedMode = false }) => {
  ...
  if (!showDetailedMode) {
    return (
      <div id={`appreciation-grid-badge-${property.id}`} className="mt-2 flex items-center justify-between w-full border border-dashed border-slate-200/60 p-2 rounded-xl bg-slate-50/50">
        <div className="flex flex-col">
          <span className="text-[8px] font-mono font-bold text-slate-400 uppercase tracking-wider block">Appreciation Trend</span>
          <span className="text-[10px] font-bold text-slate-800 flex items-center gap-0.5 mt-0.5">
            {trend.trendStatus === 'hyper' && <TrendingUp className="w-3.5 h-3.5 text-emerald-500 shrink-0" />}
            {trend.trendStatus === 'steady' && <TrendingUp className="w-3.5 h-3.5 text-sky-500 shrink-0" />}
            {trend.trendStatus === 'stagnant' && <Minus className="w-3.5 h-3.5 text-slate-400 shrink-0" />}
            +{trend.rateYoY}% YoY
          </span>
        </div>

        <div className={`px-2 py-0.5 rounded-md font-mono text-[9px] font-bold border ${colorClass}`}>
          {trend.trendStatus === 'hyper' ? '🔥 Hyper' : trend.trendStatus === 'steady' ? '📈 Steady' : '⏸ Mature'}
        </div>
      </div>
    );
  }
  ...
  const height = 55;
  const padding = 6;
  const points = trend.historicalPrices.map((v, i) => {
    const x = padding + (i / (trend.historicalPrices.length - 1)) * (width - padding * 2);
    const y = height - padding - ((v.price - minVal) / range) * (height - padding * 2);
    return `${x},${y}`;
  }).join(' ');

  return (
    <div id={`appreciation-detailed-gauge-${property.id}`} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          <span className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-wide block">Market Index Target</span>
          <h4 className="font-display font-semibold text-slate-900 text-xs">Appreciation Level</h4>
        </div>
        
        <div className="flex flex-col items-end">
          <span className={`text-[11px] font-black tracking-tight flex items-center gap-1 px-2 py-1 rounded-lg border font-mono ${colorClass}`}>
            {trend.trendStatus === 'hyper' && <TrendingUp className="w-3.5 h-3.5 text-emerald-500 shrink-0 animate-pulse" />}
            {trend.trendStatus === 'steady' && <TrendingUp className="w-3.5 h-3.5 text-sky-500 shrink-0" />}
            {trend.trendStatus === 'stagnant' && <Minus className="w-3.5 h-3.5 text-slate-400 shrink-0" />}
            +{trend.rateYoY}% YoY
          </span>
          <span className="text-[8.5px] font-mono text-slate-400 font-semibold mt-0.5 uppercase tracking-wider">{trend.trendLabel}</span>
        </div>
      </div>
      ...
      <div className="relative pt-1">
        <div className="bg-white border border-slate-150 rounded-xl p-3 flex flex-col justify-between items-center">
          ...
          <div className="w-full h-14 relative flex items-center justify-center mt-1">
            <svg className="w-full h-full overflow-visible animate-fade-in" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
              <defs>
                <linearGradient id={gradId} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={gradStopColor} stopOpacity="0.25" />
                  <stop offset="100%" stopColor={gradStopColor} stopOpacity="0.01" />
                </linearGradient>
              </defs>
          ...
          <div className="w-full grid grid-cols-5 gap-1 mt-3 border-t border-slate-100 pt-2 text-[8px] font-mono text-center text-slate-400">
            {trend.historicalPrices.map((v, i) => {
              const compactVal = v.price >= 10000000 
              ...
              return (
                <div key={v.year} className="flex flex-col justify-between">
                  ...
                  <span className="text-slate-800 font-extrabold font-sans mt-0.5 text-[8px] whitespace-nowrap">{compactVal}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="flex gap-2 items-start bg-white p-2.5 rounded-xl border border-slate-150 text-[10.5px]">
        <span className="mt-0.5 shrink-0 block">
          {trend.trendStatus === 'hyper' && <Sparkles className="w-3.5 h-3.5 text-amber-500 animate-pulse" />}
          {trend.trendStatus === 'steady' && <CheckCircle2 className="w-3.5 h-3.5 text-sky-500" />}
          {trend.trendStatus === 'stagnant' && <Info className="w-3.5 h-3.5 text-slate-400" />}
        </span>
        <div className="space-y-0.5 text-slate-600 leading-relaxed">
          <span className="font-bold text-slate-800 text-[11px] block">
            {trend.trendStatus === 'hyper' ? 'Hyper-Growth Corridor' : trend.trendStatus === 'steady' ? 'Established Strong Asset' : 'Mature Secondary Asset'}
          </span>
...
export const ListingsPageView: React.FC<ListingsPageViewProps> = ({
  ...
  const handleToggleCompare = (propertyId: string, checked: boolean) => {
    if (checked) {
      if (compareIds.length >= 3) {
        setCompareWarning("Side-by-side comparison is limited to max 3 items. Please uncheck another first.");
  ...
  return (
    <div id="listings-view" className="space-y-8 animate-fade-in text-slate-800">
      ...
      <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 border-b border-slate-100 pb-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-slate-100 p-1 rounded-xl flex items-center border border-slate-200">
            ...
            >
              <Compass className="w-3.5 h-3.5 text-current animate-spin-slow" /> Full Map
            </button>
          </div>
        </div>
      </div>
      ...
      <div id="listings-display-block" className={viewMode === 'Map' ? "col-span-1 lg:col-span-4" : "col-span-1 lg:col-span-3"}>
          {viewMode === 'Grid' && (
            /* SIMPLE GRID SECTION */
                      ...
                      <div className="relative h-48 overflow-hidden bg-slate-50">
                        ...
                        <div className="absolute top-3 left-3 flex flex-col gap-1">
                          ...
                          <span className="bg-emerald-700/90 text-white border border-emerald-600/25 font-mono text-[8px] font-extrabold py-0.5 px-1.5 rounded-sm uppercase shrink-0 flex items-center gap-0.5 shadow-xs">
                            <ShieldCheck className="w-2.5 h-2.5 text-emerald-300 shrink-0" /> Verified Deal
                          </span>
                        </div>
                        <button
                          type="button"
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between">
                          <div>
                            ...
                            <p className={`text-[10px] truncate flex items-center justify-between gap-1 mt-0.5 ${isSelectedOnMap ? 'text-emerald-850 font-medium' : 'text-slate-500'}`}>
                              <span className="truncate">{p.subLocality} • {p.bedrooms} BHK</span>
                              <span className={`text-[8.5px] font-mono font-bold px-1 py-0.1 select-none rounded border whitespace-nowrap inline-block shrink-0 leading-tight ${
                                getAppreciationTrend(p).trendStatus === 'hyper'
                                  ? 'text-emerald-700 bg-emerald-50 border-emerald-150'
                                  : getAppreciationTrend(p).trendStatus === 'steady'
                          ...
                          <div className="flex items-center justify-between">
                            <span className={`text-[11px] font-bold shrink-0 ${isSelectedOnMap ? 'text-emerald-700' : 'text-slate-900'}`}>
                              {getPriceLabel(p)}
                            </span>
                            <div className="flex items-center gap-1.5 min-w-0">
                              <label className="flex items-center gap-1 cursor-pointer text-[9px] font-mono font-bold text-neural-500 hover:text-slate-900 select-none bg-neutral-50 hover:bg-neutral-100 px-1.5 py-1 rounded border border-neutral-150/40 transition-all shrink-0">
                                ...
                                />
                                <span>{compareIds.includes(p.id) ? 'Comp' : 'Compare'}</span>
                              </label>
                              ...
                              <button
                                ...
                                className={`text-[9px] font-semibold hover:underline shrink-0 ${
                                  isSelectedOnMap ? 'text-emerald-700 font-bold' : 'text-emerald-600'
                                }`}
                              >
                                Explore →
                              </button>
          ...
          {viewMode === 'Map' && (
            ...
            <div id="full-screen-map-view" className="relative w-full h-[650px] bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl flex flex-col lg:flex-row animate-fade-in text-slate-800">
              ...
              <div className="absolute top-4 left-4 z-20 w-72 bg-white/95 border border-slate-200/80 backdrop-blur-md rounded-2xl shadow-xl p-4 space-y-4 max-h-[90%] overflow-y-auto">
                <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                  <h3 className="font-display font-bold text-xs text-slate-900 uppercase tracking-wide flex items-center gap-1.5">
                    <Compass className="w-4 h-4 text-emerald-600 animate-spin-slow" /> Map Navigator
                  </h3>
                ...
                {/* 1. Keyword search */}
                <div className="space-y-1">
                  <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-tight block">Search Query</label>
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-tight block">Acquisition</label>
                ...
                {/* 3. Sublocality */}
                <div className="space-y-1">
                  <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-tight block">Sublocality</label>
                  <select
                    value={subLocality}
                    onChange={(e) => setSubLocality(e.target.value)}
                ...
                {/* 4. Beds (BHK) */}
                <div className="space-y-1">
                  <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-tight block">Beds (BHK)</label>
                ...
                {rentOrBuy !== 'Rent' ? (
                  <div className="space-y-1 pt-1.5 border-t border-slate-100">
                    <div className="flex justify-between text-[9px] font-mono">
                      <span className="text-slate-400 font-bold uppercase">Max Valuation</span>
                      <span className="font-bold text-emerald-700">₹{(maxPrice / 10000000).toFixed(2)} Cr</span>
                    </div>
                    <input
                      type="range"
                      min={10000000}
              ...
              <div className="flex-1 h-full relative bg-slate-900 overflow-hidden select-none">
                {/* Upper header statistics block */}
                <div className="absolute top-0 right-0 left-0 p-3 bg-slate-950/75 backdrop-blur-md border-b border-slate-00/50 text-white flex items-center justify-between text-xs z-10 pl-80 lg:pl-3">
                  <span className="font-mono text-[9px] text-zinc-400 uppercase tracking-widest flex items-center gap-1">
                    <Sparkles className="w-3.5 h-3.5 text-emerald-400 animate-pulse" /> Hyderabad Interactive Registry Grid Map
                  </span>
                  <span className="bg-emerald-600/20 text-emerald-300 font-mono text-[8.5px] py-0.5 px-2 rounded-md border border-emerald-500/20 uppercase font-bold">
                    Interactive Pins Map
                  </span>
                </div>

                <svg className="w-full h-full min-h-[350px]" viewBox="0 0 500 380" preserveAspectRatio="xMidYMid slice">
                  <defs>
                    <pattern id="road-pattern-full" width="40" height="40" patternUnits="userSpaceOnUse">
                      <line x1="0" y1="20" x2="40" y2="20" stroke="#182235" strokeWidth="1" />
                      <line x1="20" y1="0" x2="20" y2="40" stroke="#182235" strokeWidth="1" />
                    </pattern>
                  </defs>

                  {/* Map Grid Background */}
                  <rect width="100%" height="100%" fill="url(#road-pattern-full)" />

                  {/* District Boundaries */}
                  <polygon points="120,40 330,40 280,120 180,120" fill="rgba(16, 185, 129, 0.04)" stroke="#1e293b" strokeWidth="1" />
                  <text x="210" y="70" fill="#475569" className="text-[9px] font-sans uppercase tracking-widest font-bold pointer-events-none" opacity="0.5">Kondapur Hub</text>

                  <polygon points="330,40 480,80 440,180 340,180" fill="rgba(16, 185, 129, 0.03)" stroke="#1e293b" strokeWidth="1" />
                  <text x="375" y="110" fill="#475569" className="text-[9px] font-sans uppercase tracking-widest font-bold pointer-events-none" opacity="0.5">Jubilee Hills Elite</text>

                  <polygon points="180,120 340,120 310,230 200,210" fill="rgba(14, 165, 233, 0.03)" stroke="#1e293b" strokeWidth="1" />
                  <text x="235" y="155" fill="#475569" className="text-[9px] font-sans uppercase tracking-widest font-bold pointer-events-none" opacity="0.5">Madhapur Tech</text>

                  <polygon points="80,210 200,210 160,320 60,280" fill="rgba(14, 165, 233, 0.04)" stroke="#1e293b" strokeWidth="1" />
                  <text x="95" y="245" fill="#475569" className="text-[9px] font-sans uppercase tracking-widest font-bold pointer-events-none" opacity="0.5">Gachibowli Area</text>

                  <polygon points="60,280 160,320 120,380 40,360" fill="rgba(16, 185, 129, 0.03)" stroke="#1e293b" strokeWidth="1" />
                  <text x="50" y="340" fill="#475569" className="text-[8px] font-sans uppercase tracking-widest pointer-events-none" opacity="0.5">Financial Dist</text>

                  {/* Outer Ring Road */}
                  <path d="M 30,120 C 150,150 250,300 450,320" fill="none" stroke="#334155" strokeWidth="2.5" strokeDasharray="6 4" />
                  <text x="140" y="190" fill="#334155" className="text-[7.5px] font-mono pointer-events-none" rotate="12">Outer Ring Road (ORR)</text>

                  {/* Lake Durgam */}
                  <ellipse cx="290" cy="240" rx="35" ry="18" fill="rgba(14, 165, 233, 0.12)" stroke="#0284c7" strokeWidth="1" />
                  <text x="262" y="242" fill="#0ea5e9" className="text-[7.5px] font-mono font-bold pointer-events-none">Durgam Lake</text>
                  ...
                  {Object.entries(mapCoordinates).map(([pId, coords]) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <g transform={`translate(${coords.x}, ${coords.y - 14})`}>
                ...
                <div className="absolute bottom-4 right-4 z-10 bg-slate-950/80 backdrop-blur-md border border-slate-850 px-3 py-1.5 rounded-xl text-[9px] text-slate-300 font-mono pointer-events-none flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span> Active Properties showing on pins
                </div>
                ...
                {filteredProperties.length === 0 && (
                  <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-6 z-30">
                    <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-xl max-w-sm text-center space-y-4">
                      ...
                      <h4 className="font-display font-bold text-white text-sm">No Pins Found</h4>
                      <p className="text-xs text-slate-400">Expand filter parameters or keyword queries to repopulate localized pins.</p>
                      <button
                        type="button"
                        onClick={handleResetFilters}
                ...
                {selectedMapProperty ? (
                  <div className="flex flex-col h-full justify-between animate-fade-in text-slate-800">
                    ...
                    <div className="p-4 border-b border-neutral-100 flex items-center justify-between">
                      <div>
                        <h4 className="font-display font-medium text-[10px] text-slate-400 uppercase tracking-widest font-bold">Property Inspector</h4>
                        <span className="font-mono text-[9px] text-zinc-400">{selectedMapProperty.subLocality} Community</span>
                      </div>
                      <button
                        type="button"
                    ...
                    <div className="p-4 overflow-y-auto flex-1 space-y-4 text-xs">
                      ...
                      <div className="relative aspect-video rounded-xl overflow-hidden bg-slate-100 border border-slate-200 shadow-sm shrink-0">
                        <img src={selectedMapProperty.images[0]} alt="" referrerPolicy="no-referrer" className="w-full h-full object-cover" />
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[10px] text-slate-400 truncate leading-tight">{selectedMapProperty.location}</p>
                      </div>
                      ...
                      <div className="bg-emerald-50/60 border border-emerald-100 rounded-xl p-3 flex justify-between items-center text-xs">
                        <span className="font-mono text-[9px] text-slate-450 uppercase tracking-wider font-bold">Registry Price</span>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[10.5px] font-mono">
                        <div className="bg-slate-50 border border-slate-150 rounded-lg p-2.5 flex flex-col justify-center">
                          <span className="text-[8.5px] text-neutral-400 uppercase tracking-tight flex items-center gap-0.5"><Square className="w-2.5 h-2.5 shrink-0" /> Carpet Area</span>
                          <span className="font-bold text-slate-800 mt-0.5">{selectedMapProperty.sizeSqFt.toLocaleString()} sqft</span>
                        </div>
                        <div className="bg-slate-50 border border-slate-150 rounded-lg p-2.5 flex flex-col justify-center">
                          <span className="text-[8.5px] text-neutral-400 uppercase tracking-tight flex items-center gap-0.5"><Compass className="w-2.5 h-2.5 shrink-0" /> Vastu Side</span>
                          <span className="font-bold text-slate-700 mt-0.5">{selectedMapProperty.facing} Side</span>
                        </div>
                      </div>

                      {/* Appreciation Trend Detailed visual gauge representation */}
                      <AppreciationTrendGauge property={selectedMapProperty} showDetailedMode={true} />

                      {/* Description excerpt text */}
                      <div className="space-y-1">
                        <span className="text-[8.5px] font-mono text-neutral-400 uppercase tracking-tight block font-bold">Description</span>
                        <p className="text-[10.5px] text-neutral-500 leading-relaxed line-clamp-4">{selectedMapProperty.description}</p>
                      </div>

                      {/* Verified amenities list */}
                      <div className="space-y-1.5 pt-1.5 border-t border-slate-100">
                        <span className="text-[8.5px] font-mono text-neutral-400 uppercase tracking-tight block font-bold">Verifiable Perks</span>
                        <div className="flex flex-wrap gap-1">
                          {selectedMapProperty.amenities.slice(0, 3).map((amenity, amIdx) => (
                            <span key={amIdx} className="text-[8.5px] text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200/50 block truncate max-w-[200px]">
                              {amenity}
                            </span>
                          ))}
                    ...
                    <div className="p-4 border-t border-slate-150 bg-slate-50/60 space-y-2">
                      <div className="flex items-center justify-between">
                        <label className="flex items-center gap-1.5 cursor-pointer text-[10px] font-mono font-bold text-neutral-550 select-none hover:text-slate-900">
                          <input
                            type="checkbox"
                            checked={compareIds.includes(selectedMapProperty.id)}
                            onChange={(e) => handleToggleCompare(selectedMapProperty.id, e.target.checked)}
                            className="rounded border-neutral-300 text-emerald-650 focus:ring-emerald-500 w-3.5 h-3.5 accent-emerald-600 cursor-pointer"
                          />
                          <span>{compareIds.includes(selectedMapProperty.id) ? 'Marked for Comparison' : 'Side-by-Side Compare'}</span>
                        </label>
                      </div>
                      ...
                      >
                        Explore Property Details <ArrowRight className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="h-full flex flex-col justify-center items-center text-center p-6 text-slate-400 space-y-3">
                    ...
                    <div>
                      <p className="font-bold text-xs text-slate-800 font-display">Select Map Pin</p>
                      <p className="text-[9.5px] text-slate-400 leading-normal max-w-[190px] mx-auto mt-1">
                        Pinpoints represent vetted Hyderabad listings under RERA. Tap an active pin to trigger high-fidelity virtual diagnostics.
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-4 items-center py-2.5 border-b border-neutral-100 text-sm gap-4">
                    ...
                    {compareIds.map((cid) => {
                      ...
                      return (
                        <div key={cid} className="font-display font-extrabold text-emerald-800 text-base">
                          {getPriceLabel(p)}
                        </div>
                      );
                    })}
                    ...
                    {compareIds.map((cid) => {
                      ...
                      return (
                        <div key={cid} className="text-neutral-850 font-mono font-medium">
                          ₹{valRate.toLocaleString('en-IN')} / sqft {p.rentOrBuy === 'Rent' ? 'rent' : ''}
                        </div>
                      );
                    })}
                    {Array.from({ length: 3 - compareIds.length }).map((_, idx) => <div key={idx} className="text-neutral-300">-</div>)}
                  ...
                  <div className="grid grid-cols-4 items-center py-2.5 border-b border-neutral-100 text-sm gap-4">
                    <div className="font-mono text-xs font-bold text-neutral-400 uppercase tracking-wider">Appreciation YoY</div>
                    {compareIds.map((cid) => {
                      const p = properties.find((item) => item.id === cid);
                      if (!p) return <div key={cid} className="text-neutral-300">-</div>;
                      const trend = getAppreciationTrend(p);
                      let tClass = 'text-emerald-700 bg-emerald-50 px-2' + ' ' + 'py-1 rounded inline-flex items-center gap-1 font-mono font-bold text-[11px] self-start border border-emerald-150';
                      if (trend.trendStatus === 'steady') tClass = 'text-sky-700 bg-sky-50 px-2 py-1 rounded inline-flex items-center gap-1 font-mono font-bold text-[11px] self-start border border-sky-150';
                      else if (trend.trendStatus === 'stagnant') tClass = 'text-slate-650 bg-slate-50 px-2 py-1 rounded inline-flex items-center gap-1 font-mono font-bold text-[11px] self-start border border-slate-200';
                      return (
                        <div key={cid} className={tClass}>
                          {trend.trendStatus === 'hyper' && <TrendingUp className="w-3.5 h-3.5 text-emerald-500" />}
                          {trend.trendStatus === 'steady' && <TrendingUp className="w-3.5 h-3.5 text-sky-500" />}
                          {trend.trendStatus === 'stagnant' && <Minus className="w-3.5 h-3.5 text-slate-400" />}
                          +{trend.rateYoY}% YoY ({trend.trendStatus === 'hyper' ? 'Hyper' : trend.trendStatus === 'steady' ? 'Steady' : 'Mature'})
                        </div>
                      );
                    })}
                    {Array.from({ length: 3 - compareIds.length }).map((_, idx) => <div key={idx} className="text-neutral-300">-</div>)}
      ...
      {compareWarning && (
        <div id="compare-warning-toast" className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-md bg-amber-50 text-amber-905 border border-amber-200 p-4 rounded-xl shadow-xl flex items-center gap-3 animate-fade-in">
